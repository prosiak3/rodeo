/*
  # Fix category column reference in auto order functions

  ## Problem
  Functions use p.category but products table has p.display_category

  ## Solution
  Replace p.category with p.display_category in generate_auto_order_suggestion function
*/

-- Drop existing function first
DROP FUNCTION IF EXISTS generate_auto_order_suggestion(uuid, integer);

-- Create generate_auto_order_suggestion function with correct column name
CREATE OR REPLACE FUNCTION generate_auto_order_suggestion(
  p_store_id uuid,
  p_analysis_period_days integer DEFAULT 180
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  PERFORM calculate_product_order_stats(p_store_id, p_analysis_period_days);
  ...
  WHERE o.store_id = p_store_id
    ...
    AND o.sent_at >= NOW() - make_interval(days => p_analysis_period_days);
  ...
  RETURN jsonb_build_object(
    ...
    'metadata', jsonb_build_object(
      ...
      'analysis_period_days', p_analysis_period_days,
