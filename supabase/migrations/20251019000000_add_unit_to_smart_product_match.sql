/*
  # Add unit field to smart_product_match function

  1. Changes
    - Update smart_product_match to return product unit
    - This allows voice recognition to use correct unit (kg vs szt)

  2. Notes
    - Only modifies return type and SELECT clause
...
BEGIN
  ...
  WITH learned_matches AS (
    ...
    GROUP BY p.id, p.name, p.index, p.unit
    HAVING COUNT(*) >= 2
  ),
  exact_matches AS (
    SELECT
