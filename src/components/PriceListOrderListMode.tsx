    ...
    return (
      <div className="min-h-screen bg-gray-50 pb-20">
        <div className="bg-gradient-to-r from-amber-500 to-orange-600 text-white p-4 sticky top-0 z-10">
          <button onClick={() => setShowQuantities(false)} className="flex items-center gap-2 mb-2 text-white">
            <ArrowLeft className="w-5 h-5" />
            <span>Powrót do listy</span>
          </button>
          <h2 className="text-xl font-bold">Podaj ilości</h2>
          <p className="text-amber-100 text-sm">Produktów: {orderItems.length}</p>
        </div>
        ...
        <div className="p-4 space-y-3">
            ...
            <div key={item.productId} className="bg-white rounded-lg shadow p-4">
              <div className="mb-3">
                ...
                <p className="text-sm text-gray-600">
                  {item.unitPrice.toFixed(2)} PLN/{item.unit} • Min: {item.minQuantity} {item.unit} • Krok: {item.quantityStep} {item.unit}
                </p>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <input
                  ...
                  className="flex-1 p-3 text-lg text-center border-2 border-gray-300 rounded-lg focus:border-amber-500 focus:outline-none"
              ...
              <div className="mt-3 p-2 bg-amber-50 rounded text-center">
                ...
                <span className="text-lg font-bold text-amber-600">{item.totalPrice.toFixed(2)} PLN</span>
              </div>
            </div>
          ))}
          ...
          <div className="bg-white rounded-lg shadow p-4 sticky bottom-20">
            <div className="flex justify-between items-center mb-3">
              <span className="font-semibold text-gray-700">Suma całkowita:</span>
              <span className="text-2xl font-bold text-amber-600">{totalAmount.toFixed(2)} PLN</span>
            </div>
            <button
              onClick={saveAsDraft}
              disabled={saving}
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      <div className="bg-gradient-to-r from-amber-500 to-orange-600 text-white p-4 sticky top-0 z-10">
        ...
        <h2 className="text-xl font-bold">Buduj listę produktów</h2>
        <div className="flex items-center gap-2 mt-2">
          <ShoppingCart className="w-5 h-5" />
          <span className="text-amber-100">Na liście: {orderItems.length}</span>
        </div>
      </div>

      {orderItems.length > 0 && (
        <div className="bg-white border-b-4 border-amber-500 p-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-semibold text-gray-800">Lista produktów</h3>
            <button
              onClick={() => setShowQuantities(true)}
